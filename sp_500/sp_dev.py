...
if not df_selected_sector.empty:
        ...
        def price_plot(symbol):
                ...
                df["Date"] = df.index
                df = df.reset_index(drop=True)  # Reset index to use Date as a column
                ...
                # Use Streamlit's line_chart for simplicity
                #st.line_chart(df.set_index("Date")["Close"], width=0, height=0, use_container_width=True)

    # Optionally, you can add more details using st.altair_chart for customization
    # import altair as alt
    # chart = alt.Chart(df).mark_line().encode(
    #     x='Date:T',
    #     y='Close:Q',
